...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    ...
    case mine:
      return play_mine(state, currentPlayer, choice1, choice2, handPos);
    ...
    case smithy:
      return play_smithy(state, currentPlayer, handPos);

    case village:
      return play_village(state, currentPlayer, handPos);
...
//	if(state->discardCount[currentPlayer] == MAX_DECK){return -1;}
...
i++;
      cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer]-1];//top card of hand is most recently drawn card.
        ...
        temphand[temphandIdx]=cardDrawn;
