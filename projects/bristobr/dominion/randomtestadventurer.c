...
// from statecopy, determine how many treasures should be drawn and from where (deck or discard)
  ...
  int discardDecrease=0;;
  for(i = copy_currentDeckCount-1; i >= 0; i--){
	
    card = stateCopy.deck[stateCopy.whoseTurn][i];
    if(card==copper || card==silver || card==gold){
      deckTreasure++;
	  if(deckTreasure == 2){  
	 	 deckDecrease = copy_currentDeckCount -i; // how many cards were drawn from deck before treasures were found?
	  }
    }
  }

  for(i=copy_currentDiscardCount-1; i >= 0;i--){
    card = stateCopy.discard[stateCopy.whoseTurn][i];
    if(card== copper || card==silver || card==gold){
      discardTreasure++;
	  if(deckTreasure + discardTreasure == 2){
		discardDecrease = copy_currentDiscardCount - i; // how many cards were drawn from discard before treasures were found?
      }
   }
  }
...
// if both cards are drawn from the deck
    if(deckTreasure>=2){
      if(currentDeckCount != copy_currentDeckCount- deckDecrease){
        printf("FAILED TEST:\n");
      ...
      if(currentDiscardCount != copy_currentDiscardCount + (deckDecrease -2) + 1){
        printf("FAILED TEST:\n");
        printf("\t deck has 2 treasures, discard not incremented by 1\n");
        printf("\t deck decreased by: %d\n", deckDecrease);
